...
function PastExpo({
  ...
  exhibitorsList: ExhibitorData[] | null;
  floorPlans: FloorPlanData[] | null;
  currentYear: number;
}) {
  ...
  return (
    <section className="w-full">
      ...
      <div className="flex flex-col gap-8 items-center justify-start w-full h-fit sm:pt-16">
        <div className="h-fit w-full rounded-md flex flex-col gap-8 pl-8">
          <h3 className="text-xl  lg:text-4xl font-poppins font-bold text-expoBlue ">
            {currentYear} Exhibitors
          </h3>
          ...
          <Swiper
            ...
            className="w-full h-fit md:h-[50vh] flex items-center justify-center rounded-lg"
          >
            {sortedExhibitorsList?.map((image, index) =>
              image.logo ? (
                <SwiperSlide
                  key={index}
                  className="h-full py-4 w-full flex items-center justify-center"
                  onClick={() => {
                    setOpenModal(true),
                      setModalInfo({
          ...
          >
            {sortedFloorplans?.map((image, index) => (
              <SwiperSlide
                key={index}
